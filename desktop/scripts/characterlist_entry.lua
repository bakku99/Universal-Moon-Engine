...
function updatePortraitWidget()
	...
	if wgt then
		...
		if sIdentity then
			wgt = addBitmapWidget({
				name = "portrait",
				icon = "portrait_" .. sIdentity .. "_charlist",
				w = CharacterListManager.PORTRAIT_SIZE,
				h = CharacterListManager.PORTRAIT_SIZE,
			});
		end
	end
...
function setMenuItems()
	if resetMenuItems and registerMenuItem then
		resetMenuItems();
		if Session.IsHost then
			registerMenuItem(Interface.getString("charlist_menu_whisper"), "broadcast", 4);
			if _tData and ((_tData.sUser or "") ~= "") then
				registerMenuItem(Interface.getString("charlist_menu_ring"), "bell", 2);
				registerMenuItem(Interface.getString("charlist_menu_kick"), "kick", 1);
				registerMenuItem(Interface.getString("charlist_menu_kickconfirm"), "kickconfirm", 1, 1);
			end
		elseif self.isActiveAndOwned() then
			registerMenuItem(Interface.getString("charlist_menu_afk"), "hand", 4);
			registerMenuItem(Interface.getString("charlist_menu_release"), "erase", 1);
...
function onMenuSelection(selection, subselection)
	if selection == 4 then
		...
		if Session.IsHost then
			...
			elseif selection == 1 and subselection == 1 then
