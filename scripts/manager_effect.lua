...
function getEffectsString(nodeCT, bPublicOnly)
	local aOutputEffects = {};

	-- Iterate through each effect
	for _,nodeEffect in ipairs(UtilityManager.getSortedNodeList(DB.getChildList(nodeCT, "effects"))) do
		local sEffect = EffectManager.getEffectString(nodeEffect, bPublicOnly);
		if sEffect ~= "" then
...
function removeEffect(nodeCT, sEffPatternToRemove)
	if not nodeCT or ((sEffPatternToRemove or "") == "") then
		return;
	end

	if _nDelayedUpdates > 0 then
		table.insert(_tRemoveAfterDelay, { sCTPath = DB.getPath(nodeCT), sPattern = sEffPatternToRemove });
		return;
	end
	...
	for _,nodeEffect in ipairs(DB.getChildList(nodeCT, "effects")) do
		if DB.getValue(nodeEffect, "label", ""):match(sEffPatternToRemove) then
			EffectManager.onPreRemoveEffect(ActorManager.resolveActor(nodeCT), nodeEffect);
			DB.deleteNode(nodeEffect);
			return;
		end
	end
end
-- onPreRemoveEffect(rActor, nodeEffect)
function onPreRemoveEffect()
...
function addEffect(sUser, sIdentity, nodeCT, rNewEffect, bShowMsg)
	...
	EffectManager.onPostAddEffect(ActorManager.resolveActor(nodeCT), nodeTargetEffect);

	return nodeTargetEffect;
end
-- onPostAddEffect(rActor, nodeEffect)
function onPostAddEffect()
	-- Do nothing; location to override
end
...
function expireEffect(nodeActor, nodeEffect, nExpireComp)
	...
	EffectManager.onPreExpireEffect(ActorManager.resolveActor(nodeActor), nodeEffect);
	DB.deleteNode(nodeEffect);
	local sMessage = string.format("%s ['%s'] -> [%s]", Interface.getString("effect_label"), sEffect, Interface.getString("effect_status_expired"));
	EffectManager.message(sMessage, nodeActor, bGMOnly);
	return true;
end
-- onPreExpireEffect(rActor, nodeEffect)
function onPreExpireEffect()
	-- Do nothing; location to override
end
